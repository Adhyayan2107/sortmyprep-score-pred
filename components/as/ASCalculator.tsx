...
import type { ASSubjectData, ASGrade } from '@/lib/types'
...
const AS_GRADES: ASGrade[] = ['A', 'B', 'C', 'D', 'E']
const GRADE_COLORS: Record<ASGrade, { bg: string; text: string }> = {
...
export default function ASCalculator() {
  const [subject, setSubject] = useState<ASSubjectData | null>(null)
  ...
  const handleSubjectChange = (data: ASSubjectData, file: string) => {
    setSubject(data)
  ...
  const result = subject && hasValidMark
    ? calculateASGrade(subject.components, marks, subject.sessions)
    : null
